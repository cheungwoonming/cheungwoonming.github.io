...
## LR实现
逻辑回归和线性回归一样，只需要记住，loss对w求偏导均为（梯度）：gradient_w=x·(y_predict-y)，gradient_bias = (y_predict-y).sum()，w = w+lr·gradient_w进行权重更新
```python
import numpy as np
...
## pytorch实现Attention
- 核心公式：attention_out = softmax[(Q * K.T)/d**0.5] * V
  - [![image]({{site.images}}/2025/2025-08-24-1.png)]({{site.images}}/2025/2025-08-24-1.png)
- 多头注意力机制，其实是把原本输入的emb_size，切割为num_head * hidden_size的维度
...
class MultiHeadAttention(nn.Module):
    def __init__(self, emb_size, num_head):
        ...
        self.hidden_size = emb_size // num_head
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        # 切分为多头注意力:(bs, seq_len, num_head, hidden_size)
        # 再交换seq_len和num_head纬度:(bs, num_head, seq_len, hidden_size)
        bs = Q.size(0)
        Q = Q.view(bs, -1, self.num_head, self.hidden_size).transpose(1, 2)
        K = K.view(bs, -1, self.num_head, self.hidden_size).transpose(1, 2)
        V = V.view(bs, -1, self.num_head, self.hidden_size).transpose(1, 2)
        ...
        # (bs, num_head, seq_len, hidden_size)
        att_output = torch.matmul(att_weights, V)

        # 换回seq_len和num_head纬度：(bs, seq_len, num_head, hidden_size)
        # 再合并多头结果为embed_size
        att_output = att_output.transpose(1, 2).contiguous().view(att_output.size(0), -1, self.emb_size)
        final_output = self.Wo(att_output)
...
if __name__ == "__main__":
    ...
    print("Output shape:", output.shape)  # 应输出: [2, 10, 512]
    print("Attention weights shape:", attn_weights.shape)  # 应输出: [2, 8, 10, 10]
```
...
题目2：给定一个数组，返回数组的“中心下标”，下标左边求和与右边求和的值相等
```python
