...
参考：https://blog.csdn.net/achen0511/article/details/105983444/
...
实现参考：https://www.cnblogs.com/wsine/p/5180343.html

动态规划
一维形式：有一串连续数字，从中挑选任意个不相邻的数字，求相加的最大值。比如1，2，3，4，3，最大值为1+3+3=7
结题思路：数组某个下标往前的最大值，只需要考虑当前下标值、上一个下标和上上个下标的值
...
求一个无序数组里面的两个数的下标，两数之和为目标值。返回两个下标
# 空间换时间，时间O(n)
...
求一个升序数组里面的两个数的下标，两数之和为目标值。返回两个下标
# 使用双指针算法，时间O(n)
def solution(array, target):
    left = 0
    right = len(array) - 1
...
斐波那契数列，每一位的值等于它前两位数字之和，前两位固定：0，1，1，2，3，5，8。。。
# 双指针算法，时间O(n)，空间O(1)
def solution(num):
    if num <= 1:
        return num
...
硬币排列，总共n枚硬币，排程阶梯形状，第k行必须刚好有k枚硬币，输出最大k
# 双指针算法，时间O(n)，空间O(1)
def solution(num):
    index = 0
    tmp_sum = 0
...
给定一个n*n的二维数组array，array[i][j] = 1代表结点i和结点j有连线，0则代表没有连线，同一条线上的结点为同一组结点，求共有多少组结点
import queue
...
# 优化1：直接计算赢面diff，先手的人比后手拿的多多少，减去，后手拿的比先手拿的多多少
...
# 优化2：使用动态规划，关键在于定于dp数组，数组中(i,j)为序列i-j位置上的赢面diff
